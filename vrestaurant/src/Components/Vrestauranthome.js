...
export default class Vrestauranthome extends Component {
    ...
    fetchRestaurants = (event) => {
        ...
        fetch(`https://v-restaurant.herokuapp.com/zomato/restaurants/${event.target.value}`, { method: 'GET', headers: {"Access-Control-Allow-Origin": "*"} })
            .then(response => response.json())
            .then(data => {
    ...
    handleformSubmit(e) {
        ...
        // When users gives user details post to node and get register success or failure message
            ...
            fetch('https://v-restaurant.herokuapp.com/zomato/register', requestOptions)
                .then(res => res.json())
    ...
    handleLogin(e) {
        ...
        fetch('https://v-restaurant.herokuapp.com/zomato/login', {
            method: 'POST',
            redirect: 'follow',
            headers: { 'Content-Type': 'application/json' },
    ...
    render() {
        ...
        let restaurantList = this.state.RestaurantSearchList.length && <ul className='searchresultsDropDown'>{
            this.state.RestaurantSearchList.map((item) => <li className='searchresultsDropDownList' key={item.name}>
                ...
                }} style={{ textDecoration: 'none', color: 'red', fontWeight: 'bolder', display:'block', width:'346px', textIndent:'1px', marginLeft:'19px' }}>{item.name}</Link></li>)}</ul>
